...
def calculate_angle_between_points(pointA, pointB, pointC):
    ...
    angle_in_radians = np.arctan2(pointC[1] - pointB[1], pointC[0] - pointB[0]) - np.arctan2(pointA[1] - pointB[1], pointA[0] - pointB[0])
...
if __name__ == '__main__':
    ...
    with mp_pose.Pose(min_detection_confidence= 0.5, min_tracking_confidence= 0.5) as my_pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                shoulder = [my_landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x,
                            my_landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]
                elbow = [my_landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x,
                         my_landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].y]
                wrist = [my_landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x,
                ...
                cv2.putText(my_image, str(my_angle),
            ...
            cv2.putText(my_image, 'REPS', (15,12),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1, cv2.LINE_AA)

            cv2.putText(my_image, str(counter), (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 1, cv2.LINE_AA)

            # The stage text in the box
            cv2.putText(my_image, 'STAGE', (65, 12),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

            cv2.putText(my_image, str(stage), (60, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 1, cv2.LINE_AA)

            # Draws the pose landmarks and the connections between them to the image
            mp_drawing.draw_landmarks(my_image, my_results.pose_landmarks, mp_pose.POSE_CONNECTIONS,
